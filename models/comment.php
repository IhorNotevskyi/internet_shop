...
class Comment extends Model
{
    ...
    {
        $sql = "SELECT u.login, c.*, CAST(c.cnt_like AS SIGNED) - CAST(c.cnt_dislike AS SIGNED) AS cnt_total FROM comments c 
                    LEFT JOIN users u ON u.id = c.id_user 
                    WHERE id_news = '{$id_news}' AND c.is_active = 1 
                    ORDER BY id_parent, cnt_total DESC, date_time DESC";
    ...
    public function cnt_comments($limit = 10)
    {
            ...
            if (Session::get('login') == 'root') {
                $sql .= " where is_active=0 or is_active=1";
            }
    ...
    public function cnt_comments_pub($limit = 10)
    {
        $sql = "select count(*) as COUNT from comments ";
    ...
    public function admin_get_comments($page = 0, $limit = 5)
    {
        $start = $page * $limit;
    ...
    public function admin_get_comments_pub($page = 0, $limit = 5)
    {
        $start = $page * $limit;
        ...
        $sql="select c.id_comment,c.id_parent,u.login,n.title_news,cat.category_name, c.`comment`,c.date_time,c.cnt_like,c.cnt_dislike,c.is_active from comments c 
            left join users u on u.id=c.id_user
            left join news n on n.id_news=c.id_news 
            left join category cat on cat.id_category=n.id_category where c.is_active=0 order by c.date_time desc limit {$start},{$limit}";

        $result = $this->db->query($sql);
        $result['count'] = $this->cnt_comments_pub($limit);
    ...
    public function getTopCommentators($limit = 5)
    {
    ...
    {
        $sql = "SELECT n.*, c.cnt
                FROM news n, (SELECT COUNT(*) cnt, id_news, date_time 
                              FROM comments
                              /*WHERE date_time > NOW() - INTERVAL 1 DAY*/
                              GROUP BY id_news 
                              ORDER BY 1 DESC, date_time DESC
                              LIMIT {$limit}
                             ) c
    ...
    public function vote($id_comment, $type)
    {
        ...
        if (!isset($result[0])) {
            ...
            echo json_encode(array('result' => 'Вы проголосовали за эту новость'));
            exit;
        }
    }
    ...
    public function add_comment($id_user, $id_news, $comment, $id_parent = 0, $is_active = 1)
    {
...
where id_comment ={$id_comment}";
    ...
    public function checkCommentDateTime($id)
    {
        $sql = "select date_time from comments where id_comment = {$id}";
    ...
    public function deleteChildrenComment($id)
    {
        $id = (int)$id;
        $sql = "delete from comments where id_parent = {$id}";
