...
{
    public function getListIndex($limit = 3, $order = 'rand()')
    {
        ...
        $sql .= " order by a.id_category, {$order}";

        return $this->db->query($sql);
    }

    public function getListIndexWhereIsAnalytics($limit = 2, $order = 'rand()')
    {
        $sql = "SELECT a.*, cc.category_name FROM news AS a ";
        $sql .= " left join category cc on cc.id_category = a.id_category ";
        $sql .= " WHERE a.is_published = 1 and is_analitic = 1 and a.id_news IN (SELECT id_news FROM news AS b ";
        $sql .= " WHERE b.is_published = 1 and is_analitic = 1 and b.id_category = a.id_category AND (SELECT COUNT(*) FROM news AS c WHERE c.is_published = 1 and is_analitic = 1 and c.id_news >= b.id_news AND c.id_category = b.id_category) <= $limit) ";
        $sql .= " order by a.id_category, {$order}";
    ...
    {
        $sql = "SELECT * FROM news WHERE is_published = 1 and is_analitic = 1 ORDER BY rand()";
...
//    }
    ...
    public function getNewsListByTagId($id, $page = 0, $limit = 100)
    {
        $start = $page * $limit;
        $sql = "select n.*,t1.id_tag,t1.tag_name,cat.category_name from news n
                left join tag_news t on t.id_news=n.id_news
                left join tags t1 on t1.id_tag=t.id_tag 
                left join category cat on cat.id_category=n.id_category 
                where t1.id_tag={$id} and n.is_published=1 
                order by id_category, date_news desc 
                limit {$start},{$limit}";
        $result = $this->db->query($sql);
    ...
    public function getPriceByLogin($id)
    {
        $sql = "SELECT COALESCE(new_price, price) AS price, title_news FROM news WHERE id_news = '{$id}'";
    ...
    public function getCategoryForCrumbs($id)
    {
        $sql = "select * from category where id_category = {$id}";
    ...
    public function getCategoryForCrumbsByNews($number)
    {
        $sql = "select id_category from news where id_news = {$number}";
    ...
    public function getOrdersByLoginId($loginId)
    {
        $sql = "select date_time, total_price, total_quantity, orders from custom 
    ...
    public function saveNews($userName, $userPhone, $userEmail, $userId, $totalPrice, $totalQuantity, $orders)
    {
        $orders = json_encode($orders);

        $sql = "insert into custom set
                                      user_name = '{$userName}',
    ...
    public function getPersonalIdByLogin($login)
    {
